...
class StereoBM(StereoMethod):
    ...
    def reset_defaults(self):
        ...
        self.parameters.update ({
            "Num Disparities": IntParameter("Number of disparities (pixels)", 640, 16, 640, to_valid=multiple_of_16),
            "Block Size": IntParameter("Kernel size for block matching (odd)", 11, 3, 63, to_valid=odd_only),
            "TextureThreshold": IntParameter("Minimum SAD to consider the texture sufficient", 10, 0, 100),
            "Uniqueness Ratio": IntParameter("How unique the match each for each pixel", 3, 0, 100),
            "SpeckleWindowSize": IntParameter("Speckle window size in pixels (filter small objects). 0 to disable.", 220, 0, 1000),
            "SpeckleRange": IntParameter("Speckle range (max diff within a window)", 64, 0, 64),
            "Disp12MaxDiff": IntParameter("Maximum allowable difference in the right-left check", 64, 0, 64),
...
class StereoSGBM(StereoMethod):
    ...
    def reset_defaults(self):
        ...
        # https://learnopencv.com/depth-perception-using-stereo-camera-python-c/
        self.parameters.update ({
            "Num Disparities": IntParameter("Number of disparities (pixels)", 640, 2, 640),
            ...
            "Mode": EnumParameter("Set it to StereoSGBM::MODE_HH to run the full-scale two-pass dynamic programming algorithm. It will consume O(W*H*numDisparities) bytes, which is large for 640x480 stereo and huge for HD-size pictures. By default, it is set to false .",
                                  3, ["MODE_SGBM", "MODE_HH", "MODE_SGBM_3WAY", "MODE_HH4"]),
            
            
            "P1": IntParameter("Penalty Cost (default=8*NChannels*BlockSize)", 0, 0, 2000),
            "P2": IntParameter("Penalty Cost. Must be > P1 (default=32*NChannels*BlockSize).", 1000, 0, 2000),
            
            "Uniqueness Ratio": IntParameter("How unique the match each for each pixel", 7, 0, 100),
            "SpeckleWindowSize": IntParameter("Speckle window size in pixels (filter small objects). 0 to disable.", 1000, 0, 1000),
            "SpeckleRange": IntParameter("Speckle range (max diff within a window)", 32, 0, 64),
            
            "Disp12MaxDiff": IntParameter("Maximum allowable difference in the right-left check", 64, 0, 64),
            "PreFilterCap": IntParameter("Max pre-filter output", 128, 1, 128),
        })
